...
addProduct = product => {
  ...
  btnAdd.addEventListener('click', e => {                   // On écoute l'événement click, notre callback prend un paramètre que nous avons appelé event ici
  ...
  const addproduct = { ...product, number: nb, ...product, color: color };
  const products = JSON.parse(localStorage.getItem('products')) || [];
  const index = products.map(one => one.id === id && one.color === color)
  .indexOf(product.id); 
  if (index === -1) {
